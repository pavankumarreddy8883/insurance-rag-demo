...
# NOTE: retrieval uses TF-IDF (scikit-learn) instead of neural embeddings deliberately --
# sentence-transformers + PyTorch exceeds the 512MB RAM limit on free hosting tiers
# (Render free web services, Colab-adjacent free hosts). TF-IDF has no torch dependency
# and is plenty for a demo-scale document set. See Insurance_RAG_Demo.ipynb for the
# neural-embedding + FAISS version if you're running somewhere with more memory headroom.
...
print(f"TF-IDF index built: {tfidf_matrix.shape[0]} passages, {tfidf_matrix.shape[1]} terms.")

# ---------------------------------------------------------------------------
# Step 4 -- RAG query-time pipeline
# ---------------------------------------------------------------------------
def retrieve(query: str, top_k: int = 3):
    """Sparse-retrieval stand-in: TF-IDF + cosine similarity (demo omits the dense-vector and
    knowledge-graph expansion from Section 3.2 for memory reasons on free hosting -- the
    hybrid-retrieval interface is otherwise the same as the notebook version)."""
    ...
    for idx in top_idxs:
        r = records[idx]
        results.append({**r, "score": float(sims[idx])})
    return results
...
# ---------------------------------------------------------------------------
CONFIDENCE_THRESHOLD = 0.27  # TF-IDF cosine floor, calibrated against this demo's 8-doc set:
# in-scope questions score ~0.28-0.59, out-of-scope questions (meteorite, alien abduction --
# see the "Break It" tests) score ~0.20-0.27. The margin is thin (~0.02) because TF-IDF only
# sees literal word overlap and this document set is small and shares generic vocabulary
# ("damage", "policy", "cover") across passages. This is a known limitation of the TF-IDF
# fallback, not a bug -- with a larger, more varied production document set the vocabulary
# is richer and this separation gets much cleaner. If you add/edit DOCUMENTS above, re-run
# the retrieval scores against your test questions and re-tune this threshold before presenting.
# Escalating a borderline-good question here is the intended fail-safe direction (Section 4.4 /
# "escalate on low confidence, not best guess") -- not a defect to apologize for in the room.
